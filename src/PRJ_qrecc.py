...
from data_structure import ConvDataset, ConvDataset_qrecc, ConvDataset_qrecc_rel
import os
from os import path
...
def get_test_query_embedding(args):
    ...
    test_dataset = ConvDataset_qrecc_rel(args, query_tokenizer, passage_tokenizer, args.test_file_path, add_doc_info=False)
...
def output_test_res(query_embedding2id,
    ...
    output_trec_file = oj(args.qrel_output_path, "train_dense_rel_res.trec")
    merged_data = []
    ...
    trec_res = print_trec_res(output_trec_file, args.trec_gold_qrel_file_path, args.rel_threshold, args.test_file_path, args.ori_qrel_file_path)
    #trec_res = print_trec_res(output_trec_file, args.trec_gold_qrel_file_path, args.rel_threshold, args.test_file_path)
    return trec_res

def print_trec_res(run_file, qrel_file, rel_threshold, input_query_file, ori_qrel_file):
    ...
    with open(oj("output/qrecc/dense_rel", "train_rel_label_rawq_1.json"), "w") as f:
        for key, value in res_mrr_dict.items():
            id_list = key.split('-')
            conv_id = id_list[0]
            turn_id = id_list[1]
...
if __name__ == '__main__':
    main()
    #print_res("output/topiocqa/baseline_conv/ANCE_train_topic_res.json", "datasets/topiocqa/ir_all_history_dev.json")

# python test_rel_qrecc.py --config=Config/test_rel_qrecc.toml
